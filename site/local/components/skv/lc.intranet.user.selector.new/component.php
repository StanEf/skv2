...
{
	ShowError(GetMessage("INTRANET_MODULE_NOT_FOUND").' LC');
	return;
}
...
$arParams["DISPLAY_TAB_STRUCTURE"] = isset($arParams["DISPLAY_TAB_STRUCTURE"]) && $arParams["DISPLAY_TAB_STRUCTURE"] != 'N' ? 'Y' : 'N';
...
/*echo '$arResult["LAST_USERS"]<pre>';
print_r($arResult["LAST_USERS"]);
echo '</pre>';*/

/*ПОЛУЧАЕМ ПОЛЬЗОВАТЕЛЕЙ ПРИКРЕПЛЕННЫХ К ОБЪЕКТУ В КОТОРОМ НАХОДИТСЯ ДАННЫЙ ДОКУМЕНТ*/

/*$arFilter["=IBLOCK_ID"] = $arParams["IBLOCK_ID"];	;
$arFilter["=PROPERTY_OBJECT_ID"] = $arParams["OBJECT_ID"];
$rsElements = CIBlockElement::GetList(
	array("SORT"=>"ASC"), $arFilter, false, false, array("ID", "PROPERTY_OBJECT_ID")
);
while($obElement = $rsElements->GetNextElement())
{
	$row = $obElement->GetFields();
	$object_documents[$row] = $row["PROPERTY_OBJECT_ID_VALUE"];
}

echo '1235v341134<pre>';
print_r($rows);
echo '</pre>';*/
$obUsersOnObject = ObjectUserTable::GetList(
	array(
		'select' => array(
			"USER_ID",
			"NAME" => "USER.NAME",
		...
		'filter' => array("=OBJECT_ID" => $arParams["OBJECT_ID"]),
		)

);
while($row = $obUsersOnObject->fetch()){
	$row["ID"] = $row["USER_ID"];
	$arResult["DISPLAY_TAB_OBJECT_USERS"][$row["USER_ID"]] = $row;
	$rows[] = $row;
}
/*echo '$arResult["DISPLAY_TAB_OBJECT_USERS"]<pre>';
print_r($arResult["DISPLAY_TAB_OBJECT_USERS"]);
echo '</pre>';*/
/*echo '$arResult["DISPLAY_TAB_OBJECT_USERS"]<pre>';
print_r($arResult["DISPLAY_TAB_OBJECT_USERS"]);
echo '</pre>';*/
$arResult["CURRENT_USERS"] = '';

$obUsersOnDocument = DocumentUserTable::GetList(
	array(
		'select' => array(
			"DOCUMENT_ID",
			"USER_ID",
		...
		'filter' => array("=DOCUMENT_ID" => $arParams["DOCUMENT_ID"]),
	)
);
while($row = $obUsersOnDocument->fetch()){
	$row["ID"] = $row["USER_ID"];
	$arResult["CURRENT_USERS"][$row["USER_ID"]] = $row;
	$rows[] = $row;
}
/*echo '$arResult["CURRENT_USERS"]<pre>';
print_r($arResult["CURRENT_USERS"]);
echo '</pre>';*/
